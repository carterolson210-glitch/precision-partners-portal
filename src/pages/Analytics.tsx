...
import { BarChart3, Gift, Bot, Download, Stamp } from "lucide-react";
import {
...
const Analytics = () => {
  ...
  const metricCards = [
    ...
    { label: "Templates Purchased", value: stats.purchases, icon: Download, hint: "Templates in your library" },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[var(--card-gap)] mb-[var(--card-gap)]">
        {metricCards.map((m) => (
          <DashboardCard key={m.label} title={m.label}>
            {loading ? (
              <div className="mt-2">
                <div className="h-8 w-16 bg-section-alt rounded animate-pulse" />
              </div>
            ) : (
              <>
                <p className="text-[32px] font-bold text-body-text font-display mt-2">{m.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-[var(--card-gap)]">
        ...
        <DashboardCard title="Referral Trend (Last 6 Months)">
          {loading ? (
            ...
            <ChartEmptyState
              ...
              description="Your monthly referral trend chart will appear here once you start sharing your referral link."
            />
          )}
        </DashboardCard>
        ...
        <DashboardCard title="Copilot Usage (Last 6 Months)">
          {loading ? (
            ...
            <ChartEmptyState
              icon={<Bot className="w-8 h-8 text-steel" />}
              title="No copilot data yet"
              description="Your AI copilot usage trend will appear here once you start conversations with the Engineering Copilot."
            />
          )}
        </DashboardCard>
        ...
        <DashboardCard title="PE Stamps by Discipline">
          {loading ? (
            ...
            <ChartEmptyState
              icon={<Stamp className="w-8 h-8 text-steel" />}
              title="No PE stamp data"
              description="Add your PE stamps to see a breakdown by engineering discipline."
            />
          )}
        </DashboardCard>
          ...
          {loading ? (
            ...
            <div className="space-y-4 py-4">
              {[
                ...
                { label: "Purchases", value: stats.purchases, color: "bg-gold" },
              ].map((item) => (
                <div key={item.label}>
                  ...
                  <div className="h-2 bg-section-alt rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${totalActivity > 0 ? (item.value / totalActivity) * 100 : 0}%` }}
                    />
                  </div>
                </div>
              ))}
            ...
            <ChartEmptyState
              ...
              description="Your platform activity summary will show referral, copilot, and purchase metrics as you use the platform."
            />
          )}
        </DashboardCard>
      </div>
